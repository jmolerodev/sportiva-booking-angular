 ...
 * Resuelve el nombre y apellidos del administrador del centro para mostrarlo
...
export class SoporteCliente implements OnInit, OnDestroy {
  ...
  /* Nombre completo del administrador del centro para mostrar en la cabecera del chat */
  public nombreAdmin: string = 'Soporte';
   ...
   * Una vez obtenido el centro, resuelve el nombre completo del administrador
   * para mostrarlo en la cabecera del chat en lugar del UID de Firebase.
   */
    ...
    this.subscription.add(
      this.membershipService.getMembresiasByCliente(this.clienteUid).pipe(
        ...
        if (centro) {
          ...
          /* Resolvemos el nombre completo del administrador para la cabecera del chat */
          this.resolverNombreAdmin(centro.adminUid);
          this.escucharChats();
        } else {
          this.centroId = null;
          this.adminId  = null;
  ...
  /**
   * Consulta el nodo Persons del administrador y almacena su nombre completo.
   * Utiliza ClienteService.getClienteByUid() ya que todos los usuarios comparten
   * la colección Persons en Firebase independientemente de su rol.
   * @param adminUid UID del administrador del centro
   */
  private resolverNombreAdmin(adminUid: string): void {
    this.subscription.add(
      this.clienteService.getClienteByUid(adminUid).subscribe(admin => {
        if (admin) {
          this.nombreAdmin = `${admin.nombre} ${admin.apellidos}`;
        }
      })
    );
  }
   ...
   * Detecta además el cambio de estado PENDIENTE → ACTIVO en tiempo real para
   * arrancar la escucha de mensajes y notificar al cliente sin necesidad de recargar.
   */
  private escucharChats(): void {
    ...
    this.subscription.add(
      this.soporteService.getChatsByCliente(this.clienteUid).subscribe(chats => {
        ...
        const anterior  = this.chatActual;
        this.chatActual = abierto ?? (lista.length ? lista[lista.length - 1] : null);

        /* Arrancamos la escucha si el chat ya estaba ACTIVO al cargar */
        if (this.chatActual?.estado === EstadoChat.ACTIVO && !anterior) {
          this.escucharMensajes(this.chatActual.uid);
          return;
        }

        /* Detectamos la transición PENDIENTE → ACTIVO en tiempo real para
         * arrancar la escucha de mensajes y avisar al cliente sin recargar */
        if (
          anterior?.estado === EstadoChat.PENDIENTE &&
          this.chatActual?.estado === EstadoChat.ACTIVO
        ) {
          this.snackbarService.showSuccess('¡Tu solicitud ha sido aceptada! Ya puedes chatear.');
          this.escucharMensajes(this.chatActual.uid);
        }
      })
    );
